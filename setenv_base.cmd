...
IF /I NOT "%COMPILER_TOOL%" == "VS" (
    ...
    ) ELSE IF NOT "%VS160COMNTOOLS%" == "" (
        CALL "%VS160COMNTOOLS%VsDevCmd.bat"
    ) ELSE (    
        @ECHO WARNING: Could not find VsDevCmd.bat.
        @ECHO WARNING: VISUAL STUDIO 2017/2019 DOES NOT APPEAR TO BE INSTALLED ON THIS MACHINE
...
IF NOT EXIST "%DS5ROOT%" GOTO :BAD_DS5_ARG
IF NOT EXIST "%DS5PATH%" GOTO :BAD_DS5_ARG

set NO_ADS_WRAPPER=1
set DOTNETMF_COMPILER=%COMPILER_TOOL_VERSION%
set LM_LICENSE_FILE=%DS5PATH%\license.dat
SET DS5_TOOL_PATH=%DS5PATH%
...
IF "%COMPILER_TOOL_VERSION_NUM%" == "15" (
  ...
  call "%VS150COMNTOOLS%VsDevCmd.bat"
  set "VSSDK15INSTALLDIR=%VSSDK150INSTALL%"
  GOTO :EOF
)
...
IF "%COMPILER_TOOL_VERSION_NUM%"=="16" (
  ...
  CALL "%VS160COMNTOOLS%VsDevCmd.bat"
  set "VSSDK16INSTALLDIR=%VSSDK150INSTALL%"
  GOTO :EOF
) 
