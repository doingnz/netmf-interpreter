...
#if defined(PLATFORM_ARM_MC9328)  || defined(PLATFORM_ARM_MERIDIAN)
#include <Targets\Native\Meridian\DeviceCode\Meridian.h>
#endif
//--//
...
void monitor_debug_printf( const char* format, ... )
{
    ...
    USART_Flush( ConvertCOM_ComPort(ABORT_TEXT_PORT /*USART_DEFAULT_PORT*/) );

    // write string
    USART_Write( ConvertCOM_ComPort(ABORT_TEXT_PORT /*USART_DEFAULT_PORT*/), buffer, len );
...
#define isprint(x) (x >= 0x20)

// Todo: Why another Hex Dump ...?
// Added in BP+ to dump memory in HEX format.
void monitor_hexdump(const char * label, void *ptr, int buflen) 
{  
  ...
  {    
    monitor_debug_printf("%06x: ", i);
    for (j=0; j<16; j++)       
      if (i+j < buflen)        
        monitor_debug_printf("%02x ", buf[i+j]);
      else        
        monitor_debug_printf("   ");
    monitor_debug_printf(" ");    
    for (j=0; j<16; j++)
      if (i+j < buflen)
        monitor_debug_printf("%c", isprint(buf[i+j]) ? buf[i+j] : '.');
    monitor_debug_printf("\n");
  }
}
//--//
...
#elif defined(PLATFORM_ARM_MERIDIAN)
    UINT32              URXDn_X;
    Meridian_USART&    USART = Meridian::USART( ConvertCOM_ComPort(USART_DEFAULT_PORT) );
#else
    // TODO ???
#endif
...
#elif defined(PLATFORM_ARM_MERIDIAN)
    while(USART.URXDn_X[0] & Meridian_USART::URXDn__CHARRDY);
#else
    // TODO ???
#endif
...
#elif defined(PLATFORM_ARM_MERIDIAN)
            if(USART.URXDn_X[0] & Meridian_USART::URXDn__CHARRDY)
            {
                goto StartMonitorMode;
            }
#else
...
StartMonitorMode:
#endif 
        {
            ...
            while(1)
            {
                ...
                if(URXDn_X & Meridian_USART::URXDn__CHARRDY)
                {
                    data[pos++] = (char)(URXDn_X & Meridian_USART::URXDn__DATA_mask);
    ...
    // TODO ???
                    ...
                    {        
                    case 'T':
                      RamTest( HalSystemConfig.RAM1.Base, HalSystemConfig.RAM1.Size, "SDRAM" );
                      break;
